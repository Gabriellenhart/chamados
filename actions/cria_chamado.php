...
if (!mysqli_query($conexao, "INSERT INTO chamados (tbl_cliente, localidade, assunto, descricao, data_abertura, prioridade, status) VALUES
((SELECT id_cliente FROM clientes WHERE nome_cliente like '".$_POST['nome_cliente']."'), (SELECT id_cidade FROM cidades WHERE nome_cidade like '".$_POST['localidade']."'), '".$_POST['assunto']."', '".$_POST['descricao']."', '".$_POST['data_abertura']."', '".$_POST['prioridade']."', '".$_POST['status']."')")){
